...
import { countryList } from "@/components/claim/types";

const steps = [
  { number: 1, label: "Your details", description: "Who you are and which booking the report relates to." },
  { number: 2, label: "Baggage details", description: "The references the airline gave you when you reported the bag." },
  { number: 3, label: "Payout details", description: "Where compensation is sent if your claim succeeds." },
];
...
const step2Schema = z.object({
  fileReferenceNumber: z.string().min(1, "This field is required"),
  baggageTagNumber: z.string().min(1, "This field is required"),
  itemsAffected: z.coerce.number().min(1, "This field is required"),
...
function BaggageShieldPage() {
  ...
  const form2 = useForm({
    ...
    defaultValues: {
      fileReferenceNumber: "",
      baggageTagNumber: "",
      itemsAffected: undefined as unknown as number,
      circumstances: "",
    },
  });
  ...
  if (submitted) {
    return (
      <FormShell
        ...
        description="We have received your mishandled baggage report. Our team will review it and contact you with next steps."
      >
  ...
  return (
    <FormShell
      eyebrow="Baggage Shield"
      title="Report mishandled baggage."
      description="Before completing this form regarding delayed or missing luggage, you must first inform your airline that your baggage did not arrive at your final destination. The airline will then give you tracking details, which are necessary to complete the report below."
      sidebar={
        <SidebarInfo
          items={[
            { label: "Booking confirmation", value: "The confirmation issued by the airline." },
            { label: "Flight details", value: "Booking reference and flight number." },
            { label: "Baggage references", value: "File reference number and baggage tag number from the airline." },
    ...
    >
      ...
      {step === 1 && (
        <form className="space-y-6">
          ...
          <div className="space-y-2">
            <Label className="text-sm font-semibold text-foreground">Upload your booking confirmation</Label>
            <FileDropzone
              files={form1.watch("files")}
              onChange={(files) => form1.setValue("files", files, { shouldValidate: true })}
              maxFiles={10}
              label="Upload your booking confirmation"
              description="The confirmation issued by the airline. PDF or image, up to 10 files."
      ...
      {step === 2 && (
        <form className="space-y-6">
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="fileReferenceNumber" className="text-sm font-semibold text-foreground">
                What is your file reference number?
              </Label>
              <Input
                id="fileReferenceNumber"
                {...form2.register("fileReferenceNumber")}
                className="rounded-xl border-input bg-canvas-soft"
              />
              {form2.formState.errors.fileReferenceNumber ? (
                <p className="text-xs font-medium text-destructive">
                  {form2.formState.errors.fileReferenceNumber.message}
                </p>
              ) : (
                <p className="text-xs text-muted-foreground">Issued by the airline when you filed the report.</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="baggageTagNumber" className="text-sm font-semibold text-foreground">
                What is your baggage tag number?
              </Label>
              <Input
                id="baggageTagNumber"
                {...form2.register("baggageTagNumber")}
                className="rounded-xl border-input bg-canvas-soft"
              />
              {form2.formState.errors.baggageTagNumber && (
                <p className="text-xs font-medium text-destructive">
                  {form2.formState.errors.baggageTagNumber.message}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="itemsAffected" className="text-sm font-semibold text-foreground">
              How many items of baggage were affected?
            </Label>
            <Input
              id="itemsAffected"
              type="number"
              min={1}
              {...form2.register("itemsAffected")}
              className="rounded-xl border-input bg-canvas-soft"
            />
            {form2.formState.errors.itemsAffected && (
              <p className="text-xs font-medium text-destructive">
                {form2.formState.errors.itemsAffected.message}
              </p>
            )}
          </div>

          <div className="space-y-2">
            <Label htmlFor="circumstances" className="text-sm font-semibold text-foreground">
              Please explain the circumstances of your mishandled baggage{" "}
              <span className="font-normal text-muted-foreground">(optional)</span>
            </Label>
            <Textarea
              id="circumstances"
              {...form2.register("circumstances")}
              rows={4}
              className="rounded-xl border-input bg-canvas-soft"
            />
            <p className="text-xs text-muted-foreground">
              Optional. When you landed, what the airline told you, and anything else relevant.
            </p>
          </div>
        </form>
      )}
